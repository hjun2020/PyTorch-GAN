...
int main(int argc, const char* argv[]) {
  if (argc != 2) {
    ...
    // imwrite("input.jpg", image);
    image.convertTo(image, CV_8UC3, 1.0 / 255.0);  // Convert to float and scale to [0, 1]
    ...
    cv::Mat temp_input_image(input_tensor.size(1), input_tensor.size(2), CV_32FC3, input_tensor.to(torch::kCPU).data_ptr<float>());
    ...
    cv::Mat output_image(output_tensor.size(1), output_tensor.size(2), CV_32FC3, output_tensor.to(torch::kCPU).data_ptr<float>());
    // output_image *= 255.0;
