...
parser.add_argument("--generator_epoch", type=int, default=0, help="pixel-wise loss weight")
parser.add_argument("--discriminator_epoch", type=int, default=0, help="pixel-wise loss weight")
...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer_G, step_size=50e3, gamma=0.5)
...
for epoch in range(opt.generator_epoch, opt.n_epochs):
    for i, imgs in enumerate(dataloader):
        ...
        # loss_G = opt.lambda_pixel * loss_content + loss_pixel + opt.lambda_adv * loss_GAN 
        # loss_G = loss_pixel
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            save_image(img_grid, "images/training/after_%d.png" % batches_done, nrow=1, normalize=False)

        if batches_done % opt.checkpoint_interval == 0:
            # Save model checkpoints
            torch.save(generator.state_dict(), "saved_models/generator_after_%d.pth" % epoch)
